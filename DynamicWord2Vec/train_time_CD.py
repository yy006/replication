...
nw = 20936 # number of words in vocab (11068100/20936 for ngram/nyt) (n-gramのペア数、あるいは単語の総出現回数/ユニークな単語数)
T = range(1990, 2016) # total number of time points (20/range(27) for ngram/nyt)
cuda = True # CUDAを使うかどうか

trainhead = 'data/wordPairPMI_' # location of training data
savehead = 'results/' # 保存データのディレクトリ
...
if __name__ == '__main__':
    import sys
    ITERS = 5 # イテレーション数(反復回数)
    lam = 10 # frob 正則化パラメータ
    gam = 100 # 強制正則化パラメータ
    tau = 50 # smoothing 正規化パラメータ
    r = 50 # rank
    b = nw # batch size
    emph = 1 # 非ゼロ重視パラメータ
